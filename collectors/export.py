...
_experiment_dir = '/home/jnejati/PLTSpeed/desktop_live-good3g-250runs'
...
def read_tcptrace_feature(_file, _feature):
    ...
    for index, my_server in enumerate(host_b):
        if str(feature_list_b2a[index]).strip().isdigit():
            _total.append(float(feature_list_b2a[index]))
            count += 1
        else:
            continue
    #return np.mean(_total)
    return max(_total)
...
def find_feature_value(f1, t1, _feature):
    ...
    elif _feature in tcptrace_feature_list:
        _fvalue1 = read_tcptrace_feature(f1, _feature)
    #return _fvalue1/_tvalue1 
    return _fvalue1, _tvalue1 
...
tcptrace_feature_list = ['throughput_b2a']
#perf_feature_list = ['task-clock']
feature_vector = [[] for i in range(len(perf_feature_list) + len(net_feature_list) + len(tcptrace_feature_list))]
label_vector = []
...
for _site_dir in exp_dirs:
    ...
    for _run_no1 in _site_dict.keys():
        ...
        for _feature in perf_feature_list + net_feature_list + tcptrace_feature_list:
            if cur_ratio:
                if _feature in tcptrace_feature_list:
                    _fvalue1, _tvalue1 = find_feature_value(_tcptrace_file1, _analysis_file1, _feature)
                else:
                    _fvalue1, _tvalue1 = find_feature_value(_feature_file1, _analysis_file1, _feature)
                #print(_feature, _feature_file1)
                if _fvalue1 > 0:
                    i += 1
                    feature_vector[i].append(_fvalue1)
            else:
                continue
        time_vector.append(_tvalue1)
    ...
    with open('./export/desktop_good3g-controlled50runs/' + _site_name + '.csv', 'w') as _outf:
        for _feature in perf_feature_list + net_feature_list + tcptrace_feature_list:
            _outf.write(_feature + ', ')
        _outf.write('similarity, time \n')
        for _index, _cur_feature_vector in enumerate(feature_vector):
            for  _f in _cur_feature_vector:
                _outf.write(str(_f) + ', ')
            _outf.write(str(label_vector[_index]) + ', ')
            _outf.write(str(time_vector[_index]) + ' \n')
    print(label_vector)
    print(_site_name)
