    ...
    for _run_no1 in _runs:
        for _run_no2 in _runs:
            if not _run_no1 == _run_no2:
                ...
                ratio_dict.setdefault(_site_name + str(_run_no1 + _run_no2), []).append(cur_ratio)
                for _feature in perf_feature_list:
                    #_fvalue = find_feature_ratio(_feature_file1, _feature_file2, _analysis_file1, _analysis_file2,  _feature)
                    _fvalue = find_tcpfeature_ratio(_tcptrace_file1, _tcptrace_file2, _feature)
                    if _fvalue:
                        feature_dict.setdefault(_site_name + str(_run_no1 + _run_no2) + '!!' +  _feature, []).append(_fvalue)
                    else:
                        continue
    for _feature in perf_feature_list:
        _cur_feature = []
        ...
        for k, v in feature_dict.items():
            if k.split('!!')[1] == _feature:
                _cur_feature.append(v)
                _cur_load.append(ratio_dict[k.split('!!')[0]])
        _cor = pearsonr(_cur_feature, _cur_load)[0]
        print(_site_name, _feature, _cor)
