...
i= -2
feature_vector = [[] for i in range((2 * len(perf_feature_list)) + (2 * len(net_feature_list)))]
label_vector = []
        ...
        for _run_no1 in _runs:
            for _run_no2 in _runs:
                if not _run_no1 == _run_no2:
                    ...
                    if _fvalue1> 0.0  and _fvalue2> 0.0:
                        if not _DONE_RATIO:
                            if cur_ratio:
                                label_vector.append(cur_ratio)
                        feature_vector[i].append(_fvalue1)
                        feature_vector[i+1].append(_fvalue2)
