...
def main():
    ...
    with open(config_file, 'r') as f:
        ...
        _path =  os.path.join('/Desktop/Mallesh/PLTSpeed', net_profile['device_type'] + '_' + net_profile['name'])
    ...
    with open('/Desktop/Mallesh/PLTSpeed/res/' + input_file) as _sites:
        for _site in _sites:
            ...
            time.sleep(5)
            os.system('DISPLAY=:7 sudo google-chrome-stable --remote-debugging-port=9222  --disable-background-networking --enable-benchmarking --enable-net-benchmarking --start-maximized  --ignore-certificate-errors --user-data-dir=$TMPDIR/chrome-profiling --no-default-browser-check &')
            time.sleep(15)
            _site = _site.strip()
