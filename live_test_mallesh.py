...
def main():
    ...
    config_file = '/home/mallesh/Desktop/Mallesh/PLTSpeed/confs/netProfiles.json'
    ...
    with open(config_file, 'r') as f:
        ...
        _path =  os.path.join('/home/mallesh/Desktop/Mallesh/PLTSpeed', net_profile['device_type'] + '_' + net_profile['name'])
        webDnsSetup.clear_folder(_path)
    with open('/home/mallesh/Desktop/Mallesh/PLTSpeed/res/' + input_file) as _sites:
        for _site in _sites:
            #os.system('pkill chrome')
            #os.system('pkill google-chrome-stable')
            #time.sleep(5)
            #os.system('sudo -u mallesh google-chrome-stable --remote-debugging-port=9222  --disable-background-networking --enable-benchmarking --enable-net-benchmarking --start-maximized  --ignore-certificate-errors --user-data-dir=$TMPDIR/chrome-profiling --no-default-browser-check')
            #time.sleep(15)
            _site = _site.strip()
            ...
            for run_no in range(10):
                ...
                _tcpdump_cmd = ['tcpdump', '-i', 'enp0s31f6', '-s', '0','-U', '-w', _tcpdump_file, 'not', 'port', '22']
                ...
                try:
                    ...
                    _cmd = _node_cmd
                    subprocess.call(_cmd, timeout = 180)
