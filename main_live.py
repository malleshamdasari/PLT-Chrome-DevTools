...
def main():
    ...
    with open('/home/jnejati/PLTSpeed/res/' + input_file) as _sites:
        for _site in _sites:
            ...
            time.sleep(5)
            os.system('google-chrome-stable --remote-debugging-port=9222 --enable-benchmarking --enable-net-benchmarking --start-maximized  --ignore-certificate-errors --user-data-dir=$TMPDIR/chrome-profiling --no-default-browser-check &')
            time.sleep(40)
            _site = _site.strip()
            ...
            for run_no in range(1):
                ...
                _perf_cmd = ['perf', 'stat', '-x,', perf_args,  '--output', _perf_file, 'timeout', '--signal=SIGINT', '210'] 
